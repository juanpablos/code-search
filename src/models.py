...
class BOWEncoder(nn.Module):
    ...
    def forward(self, _input, input_lengths=None):
        batch_size, seq_len = _input.size()
        embedded = self.embedding(_input)  # input: [batch_sz x seq_len x 1]  embedded: [batch_sz x seq_len x emb_sz]
...
class SeqEncoder(nn.Module):
    ...
    def forward(self, _input, input_lengths=None):
        batch_size, seq_len = _input.size()
        embedded = self.embedding(_input)  # input: [batch_sz x seq_len]  embedded: [batch_sz x seq_len x emb_sz]
